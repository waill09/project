...
<style>
		body {
			...
			margin: 0; /* Assurez-vous que la marge par défaut est désactivée pour le corps */
			padding: 0; /* Assurez-vous que le rembourrage par défaut est désactivé pour le corps */
		}
		.main-frame {
...
<!-- NAVIGATION -->
	<header>
		<div class="nav" id="nav">
			<div id="learned-logo">
			<a href="index1.php"><img src="images/icon/logo.png" style="width: 120px;"></a></div>
			...
			<div class="search" id="search-switch">
				<input type="search" placeholder="Search" class="srch" id="search-input"><button id="srchbtn"onclick="performSearch()" ><img src="images/icon/search.png"></button>
			</div>
		</div>
	</header>
...
<div class="panel" id="panel">
	...
	<div class="right-side" id="right">
		<div id="quiz-container">
			...
			<div id="f6"><iframe class="quiz-frame" data-title="Java" src="https://docs.google.com/forms/d/e/1FAIpQLSdHFDm_BakVxro_zJI78OF2OLJpXgDzzaAVMHD9hptWlXBSpA/viewform?embedded=true" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe></div>
...
<script>
// Ajoutez cette fonction pour gérer la recherche en temps réel
function performSearch() {
    // Récupérer la valeur de la barre de recherche
    var searchQuery = document.querySelector('.srch').value.trim().toLowerCase();

    // Récupérer tous les éléments que vous souhaitez filtrer (dans ce cas, les quiz)
    var quizzes = document.querySelectorAll('.quiz-frame');

    // Parcourir chaque quiz et vérifier s'il correspond à la requête de recherche
    quizzes.forEach(function (quiz) {
        var quizTitle = quiz.getAttribute('data-title').toLowerCase();
        var isVisible = quizTitle.includes(searchQuery);

        // Basculer la visibilité en fonction de la correspondance avec la requête de recherche
        quiz.style.display = isVisible ? 'block' : 'none';
    });
}

// Ajoutez un gestionnaire d'événements pour l'événement "input" sur la barre de recherche
var searchInput = document.querySelector('.srch');
if (searchInput) {
